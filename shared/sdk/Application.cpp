...
Application::Function* Application::get_functions() {
    static auto functions_offset = [this]() -> std::optional<int32_t> {
        ...
        for (auto i = 0x100; i < 0x1000; i += sizeof(void*)) try {
            ...
            if (ptr == nullptr || sdk::memory::IsBadMemPtr(false, ptr, sizeof(Application::Function) * 50)) {
                continue;
            }
            ...
            for (auto j = 0; j < 1024; ++j) try {
                ...
                if (func.description == nullptr || sdk::memory::IsBadMemPtr(false, func.description, 32)) {
                    break;
                }

                if (j == 0 && func.entry == nullptr || sdk::memory::IsBadMemPtr(false, func.entry, sizeof(void*))) {
